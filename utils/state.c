...
void change_state(SOLDER_STATE_Def new_state) {
  if (sensor_val.current_state == new_state)
    return;
...
void state_update(void) {
  bool inSleep = sensor_val.sleep < 0.5f;  //放在休眠座中
  bool replcing = sensor_val.replace < 0.5f; //正在进行换加热芯操作
  bool hasHandle = sensor_val.handleType != HANDLE_NONE; //未检测到手柄

  SOLDER_STATE_Def current = sensor_val.current_state;
  ...
  case STATE_STANDBY:
    /* 待机: Handle picked up -> back to RUN */
    if (!inSleep) {
      change_state(STATE_RUN);
      break;
    }
    ...
    if (config_val.sleep_timeout_min > 0.5f) {
      if (millis() - s_standby_start_ms >= (uint32_t)(config_val.sleep_timeout_min * 60000.0f)) {
  ...
  case STATE_REPLACE:
    ...
    if (!replcing) {
      if (inSleep) {
        change_state(STATE_SLEEP); /* 在架子上 -> 休眠 */
      } else {
        change_state(STATE_RUN);   /* 不在架子上 -> 运行 */
      }
    }
    break;
