...
static void spi_write_data8(uint8_t data) {
  ...
  u32Timeout = 0xFFFFU;
  while ((SPI_GetStatus(TFT_SPI_UNIT, SPI_FLAG_TX_BUF_EMPTY) == RESET) && (u32Timeout > 0U)) {
    u32Timeout--;
  }
...
void lcd_init(void) {
  ...
  GPIO_SetFunc(TFT_MOSI_PORT, TFT_MOSI_PIN, GPIO_FUNC_40); /* PB5 -> SPI3_MOSI */
...
#ifdef USE_DMA
  lcd_dma_fill(color, (uint32_t)w * h);
#else
...
void lcd_fill_screen(uint16_t color) { lcd_fill_rect(0, 0, TFT_WIDTH, TFT_HEIGHT, color); }

void lcd_draw_hline(uint16_t x, uint16_t y, uint16_t w, uint16_t color) { lcd_fill_rect(x, y, w, 1, color); }
 ...
 */
void lcd_draw_ch_char(uint16_t x, uint16_t y, const uint8_t *utf8_code, uint16_t color, uint16_t bgColor,
                      const CHSFont *font) {
  if (utf8_code == NULL || font == NULL || font->charset == NULL || font->data == NULL)
    return;
  ...
  while (*ptr != '\0') {
    ...
    if ((uint8_t)ptr[0] == utf8_code[0] && (uint8_t)ptr[1] == utf8_code[1] && (uint8_t)ptr[2] == utf8_code[2]) {
  ...
  uint16_t end_x = x + font->width - 1;
  uint16_t end_y = y + font->height - 1;

  // 完全在屏幕外则放弃绘制
  if (end_x < 0 || end_y < 0 || end_x >= TFT_WIDTH || end_y >= TFT_HEIGHT) {
    return;
  }

  // 4. 正式向屏幕液晶屏开窗
  lcd_set_window(x, y, end_x, end_y);
 ...
 * @param  font : 传入你在之前实现的英文
 * GFXfont，汉字库直接内部硬编码绑定或按需扩展
 */
void lcd_draw_chinese(uint16_t x, uint16_t y, const char *text, uint16_t color, uint16_t bgColor, const CHSFont *chFont,
                      const GFXfont *font) {
  ...
  while (*text != '\0') {
    ...
    if ((*text & 0xE0) == 0xE0) {
      ...
      lcd_draw_ch_char(cur_x, cur_y, utf8_buf, color, bgColor, chFont);
...
uint8_t lcd_draw_char(uint16_t x, uint16_t y, const char c, uint16_t color, uint16_t bgColor, const GFXfont *font) {
  ...
  // 1. 黄金比例自适应：大字小字通用，一步将左上角 y 转化为对应的逻辑基线
  int16_t baseline_y = (int16_t)y + (font->yAdvance * 4) / 5; // 取行高的 80% 作为上沿落差

  // 2. 【有符号计算】：用 int32_t 承接，防止负数偏移量（Offset）导致无符号下溢
  int16_t start_x = (int32_t)x + g->xOffset;
  int16_t start_y = baseline_y + g->yOffset;
  int16_t end_x = start_x + g->width - 1;
  int16_t end_y = start_y + g->height - 1;

  // 3. 【严格边界拦截】：完全在屏幕外的，坚决不绘制
  if (end_x < 0 || end_y < 0 || start_x >= TFT_WIDTH || start_y >= TFT_HEIGHT) {
    return g->xAdvance;
  }

  // 4. 正式向硬件屏幕开窗
  lcd_set_window((uint16_t)start_x, (uint16_t)start_y, (uint16_t)end_x, (uint16_t)end_y);
  ...
  for (uint16_t j = 0; j < g->height; j++) {
    for (uint16_t i = 0; i < g->width; i++) {
      ...
      current_byte <<= 1; // 判断完当前像素后，再为下一个像素腾出最高位
      bit_count--;
    }
  }
...
void lcd_draw_string_right(uint16_t y, const char *text, uint16_t color, uint16_t bgColor, const GFXfont *font) {
  ...
  // 2. 根据屏幕总宽（TFT_WIDTH）计算居中的起始 X 坐标
  int32_t start_x = TFT_WIDTH - total_width;
  if (start_x < 0) {
    start_x = 0; // 防止超出左边界
  }
