...
void gui_update_display(void) {
  ...
  /* Title */
    ...
    if (st == STATE_SLEEP || st == STATE_HALTED ||
      ...
      lcd_draw_string(4, 6, "SET: ---", C_GRAY, C_TITLE_BG, &DefaultFont);
    } else {
  ...
  /* Power bar */
    ...
    if (st == STATE_SLEEP || st == STATE_HALTED ||
      ...
      lcd_draw_string(bx + 8, by + 6, (st == STATE_HALTED) ? "H" : "Z", C_BLACK,  C_ORANGE, &DefaultFont);
  ...
  if (sensor_val.handleType == HANDLE_NONE) {
    ...
    snprintf(buf, sizeof(buf), "%2.0fW/%2d%%", w, duty);
  ...
  snprintf(buf, sizeof(buf), "P:%3.0f D:%2d%% I:%2.1f T:%3.0f/%3.0f", 
    sensor_val.power_req, duty, sensor_val.current, sensor_val.temp_avg,PID_setpoint);
  
  lcd_draw_string(4, DBG_Y + 2, buf, C_GRAY, C_BLACK, &FreeSans6pt7b);
